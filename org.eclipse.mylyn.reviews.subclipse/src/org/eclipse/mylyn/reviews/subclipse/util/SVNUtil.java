...
public class SVNUtil {
	 ...
	 */
	public static List<Changeset> getChangesets(IProgressMonitor monitor, String repositoryLocation, String from,
			String to, String path) {

		logger.info("Get changesets: " + repositoryLocation + ":" + from + ":" + to + ":" + path);
		try {
			ISVNRemoteResource resource = org.tigris.subversion.subclipse.core.SVNProviderPlugin.getPlugin()
					.getRepository(repositoryLocation).getRootFolder();
			...
			logger.info("[SVNUtil] Fetch revisions from: " + startRev + " to: " + endRev + " for resource "
					+ resource.getName());

			GetLogsCommand logsCmd = new GetLogsCommand(resource, SVNRevision.HEAD, startRev, endRev, false, 0, null,
					false);
	...
	private static List<Changeset> getChangesetsFromLogEntries(ILogEntry[] entries, String repositoryLocation) {
		...
		for (ILogEntry entry : entries) {
			...
			change.setFiles(convertChangePathsToFiles(entry.getLogEntryChangePaths()));
	...
	/**
	 * Helper method for removing the projectname for the given url.
	 * 
	 * @param url
	 *            Url that contains the project name.
	 * @return
	 */
	...
	private static List<File> convertChangePathsToFiles(LogEntryChangePath[] paths) {
	...
	/**
	 * This method returns the head revision for the given repository.
	 * 
	 * @param monitor
	 ...
	 * @return
	 */
	public static Long getLastRevision(IProgressMonitor monitor, String repositoryLocation) {
		...
		try {
			...
			if (logsCmd.getLogEntries().length > 0) {
				return logsCmd.getLogEntries()[logsCmd.getLogEntries().length - 1].getRevision().getNumber();
			} else {
